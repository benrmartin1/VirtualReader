...
//      }
    ...
    void Update() {
      ...
      GameObject objectLookingAt = mainCamera.GetComponent<CameraScript>().ObjectLookingAt();
            if (objectLookingAt != null)
            {
                print(objectLookingAt.name);
            }
       if (objectLookingAt != null && transform.gameObject == objectLookingAt)
            {
                print(objectLookingAt.name);
    ...
    private void transformDoubleAnchor() {
      ...
      if (_allowScale) {
        _anchor.localScale = Vector3.one * Vector3.Distance(_pinchDetectorA.Position, _pinchDetectorB.Position) * factor;
      }
    }
